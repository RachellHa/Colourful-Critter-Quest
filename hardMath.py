...
class HardMath:
    '''
            ...
            Purpose: Class for hard level of the Math Mania game, runs 10 questions
    '''

    def __init__(self, screen, game_state):
        '''
            Initialize HardMath class.
            Randomizes equation question: multiplication and division.
        ...
        self.operation = random.choice(['multiple', 'divide'])
        # Ensure math_choice is not '0'
        self.math_choice = random.choice(
            [x for x in self.hardMath.keys() if x != '0'])
        self.math_choice2 = random.choice([x for x in self.hardMath.keys() if
                                           x != '0' and x != self.math_choice])  # Ensure math_choice2 is not '0' or equal to math_choice
        ...
        self.img3 = pygame.transform.scale(
            self.hardMath[self.math_choice], (200, 200))
        self.img4 = pygame.transform.scale(
            self.hardMath[self.math_choice2], (200, 200))
    ...
    def answer(self):
        '''
            ...
            :return: answer
        '''
        if self.operation == 'multiple':
            ans = int(self.math_choice) * int(self.math_choice2)
        else:
            self.always_positive()
            # Round to two decimal places
            ans = round(int(self.math_choice) / int(self.math_choice2), 2)
        return ans
    ...
    def multiple_choice(self):
        ...
        '''
        # convert all keys in dict into integer list
        keys_list = [int(x) for x in self.hardMath]
        other_options = []

        for i in range(18):
            if i != self.eqn_ans and i != 0:  # Ensure i is not equal to the answer or 0
                other_options.append(i)

        # Select 3 random options from available choices
    ...
    def run(self):
        ...
        operation_symbol = 'x' if self.operation == 'multiple' else '÷'
    ...
    def next_question(self):
        ...
        # Initialize math_choice2 ensuring it's not zero
        possible_choices = [x for x in self.hardMath.keys() if x != '0']
        if len(possible_choices) > 1:  # Ensure there's at least one choice other than '0'
            self.math_choice2 = random.choice(possible_choices)
        else:
            # If there's only one choice other than '0', set math_choice2 to that choice
            self.math_choice2 = possible_choices[0]

        # Check if math_choice2 is equal to math_choice and reinitialize if necessary
        while self.math_choice2 == self.math_choice:
            self.math_choice2 = random.choice(possible_choices)
